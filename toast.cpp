...
public:
  ...
  Frame(wxPoint p, wxSize s, wxString message, wxString title)
    //: wxFrame(nullptr, wxID_ANY, "Toast - Hello World!", p, s,
...
public:
  ...
  virtual bool OnInit()
  {
    ...
    logger_->set_pattern(
      "[%T][%n][%l] %v"); // "[HH:MM:SS][logger_name][level] msg"
...
#else
    auto message =
      wxString("A very long message that hopefully wraps around successfully.");
